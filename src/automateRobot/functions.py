...
DIR_PATH = "D:/robot_case/src/automateRobot/"
dictionary = {}

def get_config():
    # Check to ensure the configuration file exists and is readable.
    try:
        path = pathlib.Path(DIR_PATH + "conf.json")
...
def get_jenkins_config():
    # Check to ensure the configuration file exists and is readable.
    try:
        path = pathlib.Path(DIR_PATH + "jenkinsconfig.json")
        if path.exists() and path.is_file():
            with open(path) as config_file:
                try:
                    qtest_config = json.load(config_file)
...
def get_console_output():
    ...
    getConsoleOutputUrl = JenkinsURL + "/job/" + JenkinsJobName + "/lastBuild/consoleText"
    print(getConsoleOutputUrl)
...
def create_test_logs_json(testSuite, runName, status, note, start, end, stepLog, output):
    if start is None:
    ...
    body = {'exe_start_date': start,
            ...
            'name': runName,
            'status': status,
            'automation_content': runName,
            'attachments': attachment,
            'module_names': [testSuite, runName],
...
def parse_robot(output):
    ...
    for tag in soup.find_all('test'):
        ...
        for child in tag.find_all('kw', recursive=False):
            ...
            stepName = child['name']
            stepStatus = child.find('status')['status']
            result = child.find('doc').text
            stepLog.append(format_test_steps(stepName, stepStatus, stepCount, result))
            stepCount = stepCount + 1
        value = create_test_logs_json(testSuite, testCase, status, message, startTime, endTime, stepLog, output)
        body.append(value)
    return body
